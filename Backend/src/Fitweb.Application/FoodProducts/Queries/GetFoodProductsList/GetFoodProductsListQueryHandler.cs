...
namespace Fitweb.Application.FoodProducts.Queries.GetFoodProductsList
{
        ...
        : IRequestHandler<GetFoodProductListQuery, PagedResponse<FoodProductDto>>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var (foodProducts, totalItems) = await _foodProductRepository.GetAllAsync(paginationFilter, "Information.Name");

            var foodProductDto = _mapper.Map<List<FoodProductDto>>(foodProducts);

            return PaginationHelpers.CreatePagedResponse(foodProductDto, paginationFilter, totalItems);
        }
    }
}
